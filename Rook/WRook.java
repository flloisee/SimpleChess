package ChessGame.Rook;

import java.util.*;
import ChessGame.*;
import ChessGame.MainGame.*;

class checkWRookMove {
    ...
    public void move(Turn turn, char board[][]) {
        ...
        while (turn == Turn.White) {
            ...
            if (board[8 - row][col - 1] == '♖') {
                new checkWRookMove().dispMoves(row, col, board);
                new checkWRookMove().checkAttack(row, col, board);
                ...
                if (new checkWRookMove().checkMove(rowF, colF, board) == false) {
                ...
                System.out.println("Selected piece is not a white rook!");
                continue; 
            }
