    ...
    public void move(Turn turn, char board[][]) {
        ...
        while (turn == Turn.Black) {
            ...
            if (board[8 - row][col - 1] == '♞') {
                new checkBKnightMove().dispMoves(row, col, board);
                new checkBKnightMove().checkAttack(row, col, board);
                ...
                if (new checkBKnightMove().checkMove(rowF, colF, board) == false) {
                ...
                System.out.println("Selected piece is not a black knight!");
                continue; 
            }
