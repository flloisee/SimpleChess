package ChessGame.Queen;

import java.util.*;
import ChessGame.*;
import ChessGame.MainGame.*;

class checkWQueenMove {
    ...
    public void dispMoves(int row, int col, char[][] board) {
        ...
        try {
            for (int i = 0; i < 8; i++) {
                if (tempBoard[8 - row][col - 2 - i] == blank) {
                    tempBoard[8 - row][col - 2 - i] = '·';
                } else {
                    break;
                }
            }
        ...
        try {
            for (int i = 0; i < 8; i++) {
                if (tempBoard[8 - row][col + i] == blank) {
                    tempBoard[8 - row][col + i] = '·';
                } else {
                    break;
                }
            }
        ...
        try {
            for (int i = 0; i < 8; i++) {
                if (tempBoard[8 - row - 1- i][col - 1] == blank) {
                    tempBoard[8 - row - 1 - i][col - 1] = '·';
                } else {
                    break;
                }
            }
    ...
    public void checkAttack(int row, int col, char board[][]) {
       ...
       try {
            for (int i = 0; i < 8; i++) {
                if (tempBoard[8 - row][col - 2 - i] != blank && tempBoard[8 - row][col - 2 - i] != '·' && new TeamMate().checkTeam(tempBoard[8 - row][col - 2 - i]) == 0) {
                    tempBoard[8 - row][col - 2 - i] = 'X';
                    targeted = true;
                } else {
                    continue;
                }
        ...
        try {
            for (int i = 0; i < 8; i++) {
                if (tempBoard[8 - row][col + i] != blank & tempBoard[8 - row][col + i] != '·' && new TeamMate().checkTeam(tempBoard[8 - row][col + i]) == 0) {
                    tempBoard[8 - row][col + i] = 'X';
                    targeted = true;
                } else {
                    continue;
                }
        ...
        try {
            for (int i = 0; i < 8; i++) {
                if (tempBoard[8 - row - i][col - 1] != blank & tempBoard[8 - row - i][col - 1] != '·' && new TeamMate().checkTeam(tempBoard[8 - row - i][col - 1]) == 0) {
                    tempBoard[8 - row - i][col - 1] = 'X';
                    targeted = true;
                } else {
                    continue;
                }
        ...
        // Diagonal Axis
         // backward 
        // top
        targeted = false; // purpose is to break once it hits something, enemy piece
        try {
            for (int i = 0; i < 8; i++) {
    ...
    public void move(Turn turn, char board[][]) {
        ...
        while (turn == Turn.White) {
            ...
            if (board[8 - row][col - 1] == '♕') {
                new checkWQueenMove().dispMoves(row, col, board);
                new checkWQueenMove().checkAttack(row, col, board);
                ...
                if (new checkWQueenMove().checkMove(rowF, colF, board) == false) {
                ...
                System.out.println("Selected piece is not a white queen!");
                b.boardDisp(board);
                continue; 
            }
